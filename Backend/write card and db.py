...
def writeCard(): #Function for writing the card and db
    ...
    print()
    conn.execute("INSERT INTO ID_CARDS(Hashed_ID, text, First_name, Last_name, Time_created) VALUES (?,?,?,?,?)", [cardId, text, firstName, lastName, timeCreated]).lastrowid #Writes the information to the database
    conn.commit()
